(function (global, factory) {
  ...
  function updateVars(obj) {
    ...
    set_overlayColor =
      obj.getAttribute("data-overlay") || obj.settings.overlayColor;
    set_ratio = obj.getAttribute("data-ratio") || obj.settings.ratio;
    set_autoplay = obj.getAttribute("data-autoplay") || obj.settings.autoplay;
    set_href = obj.getAttribute("data-href") || obj.getAttribute("href");
    set_customclass =
      obj.getAttribute("data-customclass") || obj.settings.customClass;
  ...
  function close() {
    ...
    if (
      current_item.settings.onPreClose &&
      typeof current_item.settings.onPreClose === "function"
    ) {
      current_item.settings.onPreClose(
        current_item,
        gallIndex,
  ...
  function contentLoaded() {
    ...
    if (
      current_item.settings.onContentLoaded &&
      typeof current_item.settings.onContentLoaded === "function"
    ) {
  ...
  function loadInline(dest) {
    ...
    if (inlineContent) {
      content.classList.add("vbox-loading");
      newcontent =
        '<div class="vbox-inline">' + inlineContent.innerHTML + "</div>";
      content.classList.remove("vbox-loading");
      checkState("animated");
    }
  }
  ...
  function updateOverlay(destination) {
    ...
    elPreloaderInner.innerHTML = createspinner(
      spinners[destination.settings.spinner]
    );
    ...
    let titleWidth =
      destination.settings.titleStyle === "bar" ? "100%" : "auto";
    let titleRadius = destination.settings.titleStyle === "pill" ? "5em" : "0";
    let shareWidth =
      destination.settings.shareStyle === "bar" ? "100%" : "auto";
    let shareRadius = destination.settings.shareStyle === "pill" ? "5em" : "0";
    let titlebg =
      destination.settings.titleStyle === "transparent"
        ? "transparent"
        : destination.settings.toolsBackground;
    let sharebg =
      destination.settings.shareStyle === "transparent"
        ? "transparent"
        : destination.settings.toolsBackground;
    ...
    overlay.style.setProperty(
      "--vbox-tools-color",
      destination.settings.toolsColor
    );
  ...
  function navigateGall(destination) {
    if (
      !destination ||
      navigationDisabled ||
      !document.body.classList.contains("vbox-open")
    ) {
      return false;
    }
    ...
    animate({
      ...
      draw: function (progress) {
        ...
        if (progress === 1) {
          ...
          if (
            current_item.settings.onNavComplete &&
            typeof current_item.settings.onNavComplete === "function"
          ) {
            current_item.settings.onNavComplete(
              current_item,
              gallIndex,
  ...
  function open(obj) {
    ...
    if (
      obj.settings.onPreOpen &&
      typeof obj.settings.onPreOpen === "function"
    ) {
    ...
    animate({
      ...
      draw: function (progress) {
        ...
        if (progress === 1) {
          ...
          checkState("loading");
          if (
            current_item.settings.onPostOpen &&
            typeof current_item.settings.onPostOpen === "function"
          ) {
            current_item.settings.onPostOpen(
              current_item,
              gallIndex,
    ...
    overlay.addEventListener("click", function (e) {
      ...
      if (closeBtn) {
        if (
          ...
          closeBtn === e.target ||
          (current_item.settings.overlayClose &&
  ...
  function init(venobox, settings) {
    ...
    let selectors =
      settings.jQuerySelectors || document.querySelectorAll(settings.selector);
    let navigation =
      '<a class="vbox-next"><span>Next</span></a><a class="vbox-prev"><span>Prev</span></a>';
    let vbheader =
    ...
    let preloader =
      '<div class="vbox-preloader"><div class="vbox-preloader-inner"></div></div>';
    core =
